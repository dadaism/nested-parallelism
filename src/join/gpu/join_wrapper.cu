...
{	
	...
	cudaCheckError( __LINE__, cudaMalloc( (void**)&d_key1, sizeof(FLOAT_T)*(noEdgeTotal) ) );
...
void join_gpu()
{
	...
	join_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray,
										d_key1, d_value1, d_key2, d_value2, noNodeTotal);
}

void join_dual_queue_gpu()
{
	...
	if ( queue_length_l!=0 ) {
		...
		csr_join_thread_queue_kernel<<<dimGridT, dimBlockT>>>(d_vertexArray, d_edgeArray, d_work_queue_l,
	...
	if ( queue_length_h!=0 ) {
		...
		csr_join_block_queue_kernel<<<dimGridB, dimBlockB>>>(d_vertexArray, d_edgeArray, d_work_queue_h,
...
void join_shared_delayed_buffer_gpu()
{

	csr_join_shared_delayed_buffer_kernel<<<dimGrid, dimBlock>>>(	d_vertexArray, d_edgeArray, d_data,
																	d_x, d_y, noNodeTotal);
	cudaCheckError( __LINE__, cudaGetLastError() );
}

void join_global_delayed_buffer_gpu()
{
	...
	csr_join_global_delayed_buffer_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray, d_data,
	...
	csr_join_block_queue_kernel<<<dimGridB, dimBlockB>>>(	d_vertexArray, d_edgeArray, d_buffer, d_buffer_size,
															d_data, d_x, d_y, noNodeTotal );
	cudaFree(d_buffer);
	cudaFree(d_buffer_size);
}

void join_np_naive_gpu()
{
	csr_join_multidp_kernel<<<dimGrid, dimBlock>>>(	d_vertexArray, d_edgeArray, d_data,
													d_x, d_y, noNodeTotal);
	cudaCheckError( __LINE__, cudaGetLastError() );
}

void join_np_opt_gpu()
{
	int *d_buffer;
	/* prepare GPU */
	cudaCheckError( __LINE__, cudaMalloc( (void**)&d_buffer, sizeof(int)*GM_BUFF_SIZE ) );

	csr_join_singledp_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray, d_data, d_x, d_y,
													noNodeTotal, d_buffer);
	cudaCheckError( __LINE__, cudaGetLastError() );
	cudaFree(d_buffer);
}

void join_np_consolidate_gpu()
{
	int *d_buffer;
	unsigned int *d_buffer_size;
...
//	csr_join_warp_dp_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray, d_data, d_x, d_y,
//													noNodeTotal, d_buffer);

//	csr_join_block_dp_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray, d_data, d_x, d_y,
//													noNodeTotal, d_buffer);
	...
	csr_join_grid_dp_kernel<<<dimGrid, dimBlock>>>(d_vertexArray, d_edgeArray, d_data, d_x, d_y,
...
void JOIN_GPU()
{
	...
	switch (config.solution) {
		case 0:  join_gpu();	//
			break;
		case 1:  join_dual_queue_gpu();	//
			break;
		case 2:  join_shared_delayed_buffer_gpu();	//
			break;
		case 3:  join_global_delayed_buffer_gpu();	//
			break;
		case 4:  join_np_naive_gpu();	//
			break;
		case 5:  join_np_opt_gpu();	//
			break;
		case 6:  join_np_consolidate_gpu();	//
					break;
		default:
			break;
	}
