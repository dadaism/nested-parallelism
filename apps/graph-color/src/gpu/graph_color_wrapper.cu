...
void gclr_queue_dual_queue_gpu()
{
	unsigned int queue_max_length_l = QMAXLENGTH*4/5;
	unsigned int queue_max_length_h = QMAXLENGTH*1/5;
	int queue_length_l;
...
void GRAPH_COLOR_GPU()
{
	...
	switch (config.solution) {
		...
		case 7:  gclr_np_consolidate_gpu();	//
			break;
		case 8:  gclr_queue_dual_queue_gpu();	//
			break;
		case 9:  gclr_queue_shared_delayed_buffer_gpu();	//
			break;
		case 10:  gclr_queue_global_delayed_buffer_gpu();	//
			break;
		case 11:  gclr_queue_np_naive_gpu();	//
			break;
		case 12:  gclr_queue_np_opt_gpu();	//
			break;
		case 13:  gclr_queue_np_consolidate_gpu();	//
			break;
		default:
			break;
	}
