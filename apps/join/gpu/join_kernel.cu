#ifndef __JOIN_KERNEL__
#define __JOIN_KERNEL__
...
__inline__ __device__ int binary_search() {

	return -1;
}

__global__ void	join_bitmap_init_kernel( char *frontier, int node_num )
{
...
__global__ void	csr_join_thread_queue_kernel( int *ptr, int *indices, int *queue, unsigned int *queue_length,
...
__global__ void	csr_join_block_queue_kernel(int *ptr, int *indices, int *queue, unsigned int *queue_length,
											FLOAT_T *data, FLOAT_T *x, FLOAT_T *y, int node_num )
{
...
*/
__global__ void csr_join_shared_delayed_buffer_kernel(	int *ptr, int *indices, FLOAT_T *data,
...
*/
__global__ void csr_join_global_delayed_buffer_kernel(	int *ptr, int *indices,
...
*/
__global__ void join_process_neighbors( int *indices, FLOAT_T *data, FLOAT_T *x, FLOAT_T *y,
...
__global__ void csr_join_multidp_kernel(int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x,
...
#endif
     		join_process_neighbors<<<edge_num/NESTED_BLOCK_SIZE+1, NESTED_BLOCK_SIZE,0,s[threadIdx.x%MAX_STREAM_NUM]>>>(
...
__global__ void join_process_buffer( int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x, FLOAT_T *y,
...
__global__ void csr_join_singledp_kernel(int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x,
										FLOAT_T *y, int node_num, int *buffer)
{
	...
	if (threadIdx.x==0 && *block_index!=0){
		...
		nested_calls++;
#endif
		join_process_buffer<<<*block_index,NESTED_BLOCK_SIZE,0,s>>>( ptr, indices, data, x, y, node_num,
																buffer+block_offset, *block_index);
	}
}

/* thread queue with dynamic parallelism and a single nested kernel call per thread-block*/
__global__ void csr_join_warp_dp_kernel(int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x,
										FLOAT_T *y, int node_num, int *buffer)
{
	cudaStream_t s[MAX_STREAM_NUM];
	for (int i=0; i<MAX_STREAM_NUM; ++i) {
	...
	*warp_index = 0;
	...
	if (threadIdx.x%WARP_SIZE==0 && *warp_index!=0){
		...
		nested_calls++;
#endif
		join_process_buffer<<<*warp_index,NESTED_BLOCK_SIZE,0,s[threadIdx.x%MAX_STREAM_NUM]>>>( ptr, indices, data, x, y, node_num,
																buffer+warp_offset, *warp_index);
	}
}

/* thread queue with dynamic parallelism and a single nested kernel call per thread-block*/
__global__ void csr_join_block_dp_kernel(int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x,
										FLOAT_T *y, int node_num, int *buffer)
{
	cudaStream_t s;
	cudaStreamCreateWithFlags(&s, cudaStreamNonBlocking);
...
/* thread queue with dynamic parallelism and a single nested kernel call per thread-block*/
__global__ void csr_join_grid_dp_kernel(int *ptr, int *indices, FLOAT_T *data, FLOAT_T *x,
										FLOAT_T *y, int node_num, int *buffer,
										unsigned int *idx, unsigned int *count)
{
	...
	if (threadIdx.x==0) {
		...
		if ( atomicInc(count, MAXDIMGRID) >= (gridDim.x-1) ) {//
			...
			csr_join_block_queue_kernel<<<dimGridB, NESTED_BLOCK_SIZE,0,s>>>(ptr, indices, buffer, idx,
																			data, x, y, node_num );
		}
	}
}
