                                                  ...
                                                  int *buffer)
{
	...
	if (threadIdx.x%WARP_SIZE==0 && warp_index[warpId]!=0){
		...
		nested_calls++;
#endif
      	sssp_process_buffer<<<warp_index[warpId],NESTED_BLOCK_SIZE,0, s[threadIdx.x%MAX_STREAM_NUM]>>>( vertexArray, edgeArray, weightArray, costArray,
        						  		update, nodeNumber, buffer+warp_offset, warp_index[warpId]);
        }
}
                                                  ...
                                                  int *buffer, unsigned int *idx)
{
	...
	__syncthreads();
	// dump shm_buffer to global buffer and load balance
	if (threadIdx.x==0) {
		atomicInc(idx, per_block_buffer);
